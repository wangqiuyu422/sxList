  ...
  <div class="train-page">
    ...
    <div class="difficulty-tabs">
      <button
        ...
        :key="level.value"
        :class="['difficulty-tab', { active: currentDifficulty === level.value }]"
...
const difficultyLevels = [
  ...
  { label: '困难', value: 'hard' },
  { label: '短语', value: 'commonPhrases' }
]

const currentDifficulty = ref<'easy' | 'medium' | 'hard' | 'commonPhrases'>('easy')
...
const currentWords = computed(() => {
  return wordData[currentDifficulty.value] || []
})

const currentText = computed(() => {
  if (shuffledWords.value.length === 0) {
    return currentWords.value.slice(0, 10).join(' ')
  }
  return shuffledWords.value.join(' ')
})
...
function shuffleWords() {
  ...
  shuffledWords.value = words.slice(0, 10)
}

function changeDifficulty(difficulty: 'easy' | 'medium' | 'hard' | 'commonPhrases') {
